...
const AdminPendingProperties = () => {
  ...
  const fetchPendingProperties = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/properties?approvalStatus=pending', {
        headers: {
      ...
      setError('Error loading pending properties')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Review properties submitted via API</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-orange-900">
                These properties are awaiting your approval
              </p>
              <p className="text-sm text-orange-700">
                Review each property and approve or reject them
              </p>
            </div>
          </div>
        </div>
        ...
        {!loading && !error && properties.length === 0 && (
          <div className="bg-white rounded-lg shadow p-12 text-center">
            ...
            <p className="text-gray-600">No pending properties to review at the moment.</p>
          </div>
        )}
                    ...
                    <tr key={property._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            to={`/management/properties/review/${property._id}`}
        ...
        {!loading && !error && properties.length > 0 && (
          <div className="mt-6 bg-white rounded-lg shadow p-4">
            <p className="text-sm text-gray-600">
              Showing <span className="font-semibold text-gray-900">{properties.length}</span> pending {properties.length === 1 ? 'property' : 'properties'}
            </p>
          </div>
        )}
      </div>
